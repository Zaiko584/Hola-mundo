...
var commands = exports.commands = {
	...
	modchat: function (target, room, user) {
		if (!target) return this.sendReply("El chat moderado está configurado actualmente como: " + room.modchat);
		if ((user.locked || room.isMuted(user)) && !user.can('bypassall')) return this.errorReply("No puedes hacerlo mientras no puedas hablar.");
		...
		if (!room.modchat) {
			this.add("|raw|<div class=\"broadcast-blue\"><b>¡El chat moderado estaba deshabilitado!</b><br />Cualquiera puede hablar ahora.</div>");
		} else {
			var modchat = Tools.escapeHTML(room.modchat);
			this.add("|raw|<div class=\"broadcast-red\"><b>El chat moderado se estableció en " + modchat + "!</b><br />Sólo los usuarios de rango "+ modchat +" y superior pueden hablar.</div>");
		}
	...
	lockdown: function (target, room, user) {
		...
		for (var id in Rooms.rooms) {
			...
			curRoom.addRaw("<div class=\"broadcast-red\"><b>El servidor se reiniciara prontro.</b><br /> Por favor, termine sus batallas rápidamente. No se pueden iniciar nuevas batallas hasta que el servidor se restablezca en unos pocos minutos.</div>");
	...
	lockdownhelp: ["/lockdown - bloquea el servidor, lo que evita que se inicien nuevas batallas para que el servidor pueda reiniciarse. Requiere: ~"],

	prelockdown: function (target, room, user) {
		if (!this.can('lockdown')) return false;
		Rooms.global.lockdown = 'pre';
		this.sendReply("Los torneos se han deshabilitado en preparación para el reinicio del servidor.");
	...
	slowlockdown: function (target, room, user) {
		...
		for (var id in Rooms.rooms) {
			...
			curRoom.addRaw("<div class=\"broadcast-red\"><b>El servidor se reiniciará pronto.</b><br /> Por favor termine sus batallas rápidamente. No se pueden iniciar nuevas batallas hasta que el servidor se restablezca en unos pocos minutos.</div>");
		}
	...
	endlockdown: function (target, room, user) {
		...
		if (Rooms.global.lockdown === true) {
			for (var id in Rooms.rooms) {
				if (id !== 'global') Rooms.rooms[id].addRaw("<div class=\"broadcast-green\"><b>Se canceló el reinicio del servidor.</b></div>");
			}
		} else {
